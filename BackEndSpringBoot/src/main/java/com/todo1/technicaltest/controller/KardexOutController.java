...
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/kardexout")
@Api( value = "Products outputs service.", tags = { "Products outputs" })
public class KardexOutController {
	
	private KardexOutService kardexOutService;
	private static final Logger LOGGER = LogManager.getLogger(KardexOutController.class.getName());
	
	public KardexOutController(KardexOutService kardexOutService) {
		this.kardexOutService = kardexOutService;
	}	
	
	@ApiOperation(value = "Create a output.", response = BaseResponseDto.class,tags = { "Products outputs" })
	@ApiResponses(value = { 
	@ApiResponse(code = 200, message = "Product output created.",response = BaseResponseDto.class),
	@ApiResponse(code = 400, message = "Can't create product output. Validation exception", response = BaseResponseDto.class),
	@ApiResponse(code = 500, message = "Internal server error.", response = BaseResponseDto.class), 
	})
	@PostMapping(value = "/save",produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<BaseResponseDto> saveProduct(@RequestBody(required = true) KardexOutDto kardexOutDto) {
		BaseResponseDto response = new BaseResponseDto();
		HttpHeaders headers = new HttpHeaders();
		HttpStatus httpStatus = null;
		try {
			kardexOutService.saveKardexOut(kardexOutDto);
	...
	@ApiOperation(value = "Get secuence.", response = BaseResponseDto.class,tags = { "KardexOut Secuence" })
	@ApiResponses(value = { 
	...
	public ResponseEntity<BaseResponseDto> getLastSequence() {
		...
		HttpStatus httpStatus = null;
		try {
			response.setResponseBody(kardexOutService.getLastSequence());
			response.setResponseMessage(SystemMessage.STATUS_OK);
			httpStatus = HttpStatus.OK;
		} catch (Todo1CustomException e) {
			response.setStatusCode(HttpStatus.BAD_REQUEST.value());
